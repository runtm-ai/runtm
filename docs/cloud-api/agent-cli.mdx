...
| `runtm-api session create` | Create a sandbox (no prompt). Add `--agent-id` to run it as a roster agent. |
| `runtm-api session launch` | Create + fire a prompt as a background task. Add `--agent-id` to spawn a [subagent](/cloud-api/patterns/subagents). |
...
| `runtm-api agents list\|create\|update\|delete` | The agent roster (no `--type`); triggers with `--type slack\|github\|linear\|email`. `list --can-launch` narrows to what this key may launch. |
