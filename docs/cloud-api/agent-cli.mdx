...
| Authenticate to a service | Tool provider connection | A person, via `https://app.runtm.com/connect?provider=<slug>` (any scope). `tools create` only for scripts a person runs with their own secret |
