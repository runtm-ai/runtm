---
title: Context
description: "The context layers every session receives: instructions (org, user, template), skills, MCP servers, docs and knowledge sources, and the boot order they are applied in. Read when an agent ignores an instruction or cannot see a tool."
---
...
See [User Instructions](/cloud-api/context/user-instructions-get) and [Org Instructions](/cloud-api/context/org-instructions-get).

## Skills

Skills are reusable bundles that teach an agent how to do something. Each skill can include:
...
Each skill has exactly one live version. Editing it changes every template it is attached to on the next rebuild; lock a skill to prevent edits.
...
Two distinct things, on two different endpoints:

- A **provider** is the *definition*, the service, the auth method(s) it supports, and the tools/package it brings. Providers are either **managed** (runtm-seeded) or **custom** (org-created or forked). They carry **no user credentials**. Managed via [`/api/knowledge/providers`](/cloud-api/context/knowledge/list) and listed read-only in the [catalog](/cloud-api/context/knowledge/catalog).
- A **connection** (a *knowledge integration*) is a user's *stored credentials* against a provider, an API key, OAuth token, or service-account JSON, kept **encrypted** with a `status` and an org/personal scope. Managed via [`/api/knowledge/integrations`](/cloud-api/context/knowledge/create).

One provider can have **many** connections (e.g. "Production" and "Dev", or one per teammate). The API never returns the secret itself, connections expose only `has_credentials` and `status`. MCP servers follow the same split: the server is the definition, and credentials are a separate *directive connection*.
