---
title: Sessions
description: "What a session is (the sandbox an agent run happens in), its states, auto-pause and heartbeats, and how sessions relate to roster agents. Read when a run is paused, missing, or you need the session object itself."
---
...
- **`ttl_minutes`** - hard limit before the session is automatically destroyed
...
Sessions are the sandboxes where work happens. An agent is a named roster entry (a job, instructions, an evaluation rubric, a budget) that launches sessions on its default template whenever one of its [triggers](/build/how-it-works/triggers) fires. Every agent run is a session; a session opened from the dashboard is not an agent run unless it was launched for one. See [Define the job](/build/define-the-job).
