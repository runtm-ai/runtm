...
<Frame caption="Performance evaluation on the agent's Profile tab. One category per alert type, each with success criteria the grader reads verbatim.">
  <img src="/images/dashboard/agent-evaluation.png" alt="Performance evaluation section with a category, grading tag, human cost and human time" />
</Frame>
...
<Steps>
  ...
  <Step title="Stripe, as an agent-scoped connection">
    ...
    The key goes from the person straight to Runtime, never through an agent. An agent building this hands the person a `/connect` link with `agent=<risk_agent_id>`, which opens the connect dialog already set to the agent scope.
    ...
    cli-handoff: https://app.runtm.com/connect?provider=stripe&agent=<risk_agent_id>&method=api_credentials
  ...
  <Step title="BigQuery, org-wide, one dataset">
    ...
    ```text
    cli-handoff: https://app.runtm.com/connect?provider=bigquery&scope=org&method=service_account_json
    cli-verify: runtm-api tools list --provider bigquery --scope org
    ```
  ...
  <Step title="Postgres, org-wide, read-only role">
    ...
    cli-handoff: https://app.runtm.com/connect?provider=postgres&scope=org&method=api_credentials
...
<Steps>
  ...
  <Step title="Create the schedule, disabled">
    ...
    <Frame caption="The scheduled agent dialog: prompt first, then coding agent, template, schedule and post target along the bottom.">
      <img src="/images/dashboard/scheduled-agents-new.png" alt="New scheduled agent dialog with the prompt field and the harness, template, schedule and post controls" />
    </Frame>
...
<Steps>
  ...
  <Step title="Read the scorecard">
    ...
    <Frame caption="Activity > Agents: graded runs, objective hit rate, value returned and the monthly budget per agent.">
      <img src="/images/dashboard/activity-agents-scorecard.png" alt="Activity page, Agents tab, showing per-agent performance and budget" />
    </Frame>
...
<Steps>
  ...
  <Step title="Pin the network">
    ...
    <Frame caption="Network rules turn egress into deny-all plus the listed hosts.">
      <img src="/images/dashboard/guardrails-network.png" alt="Guardrails page, Network tab, with host rows" />
    </Frame>
