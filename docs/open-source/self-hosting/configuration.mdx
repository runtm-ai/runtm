...
| `BUILD_TIMEOUT_SECONDS` | `2400` | Max build time (covers the whole remote build + rolling update) |
| `DEPLOY_TIMEOUT_SECONDS` | `600` | Max deploy time |
...
See [admin commands](/open-source/self-hosting/admin-commands#rotate-pepper) for the guided rotation process.
